...
<ul class="breadcrumb">
    ...
    <li style="color: var(--text-heading); font-weight: 600;">Promo Codes</li>
</ul>
...
<div class="promo-layout">
        ...
        <form method="POST" action="{{ route('admin.promo-codes.store') }}">
            ...
            <div class="form-grid">
                ...
                <div><label>Assigned user <span>(optional)</span></label><select name="assigned_user_id">
                        <option value="">Any eligible user</option>@foreach($promoUsers as $user)<option value="{{ $user->id }}" @selected((string) old('assigned_user_id')===(string) $user->id)>{{ $user->email }}{{ $user->name ? ' — '.$user->name : '' }}</option>@endforeach
                    </select></div>
    ...
    <section class="card">
        <div class="table-header">
            <div>
                <h2>Promo Codes</h2>
                <p class="muted">Redemptions are counted only after a successful purchase.</p>
            </div>
        </div>
        <div style="overflow-x:auto">
            <table>
                ...
                <tbody>
                    @forelse($promoCodes as $promoCode)<tr>
                        ...
                        <td>{{ $promoCode->expires_at?->format('d M Y, H:i') ?? 'Never' }}</td>
                        <td><span class="badge badge-{{ $promoCode->active ? 'success' : 'warning' }}">{{ $promoCode->active ? 'Active' : 'Inactive' }}</span></td>
                        <td>
                            <div class="actions"><a class="btn btn-secondary btn-small" href="{{ route('admin.promo-codes.edit', $promoCode) }}" title="Edit {{ $promoCode->code }}"><i class="fa-solid fa-pen"></i><span>Edit</span></a>
                                <form method="POST" action="{{ route('admin.promo-codes.toggle-status', $promoCode) }}">@csrf @method('PATCH')<button class="btn btn-secondary btn-small" type="submit">{{ $promoCode->active ? 'Deactivate' : 'Activate' }}</button></form>
                                <form method="POST" action="{{ route('admin.promo-codes.destroy', $promoCode) }}" onsubmit="return confirm('Delete this promo code?')">@csrf @method('DELETE')<button class="btn btn-danger btn-small" type="submit" title="Delete {{ $promoCode->code }}"><i class="fa-solid fa-trash"></i><span>Delete</span></button></form>
                            </div>
                        </td>
                    </tr>
                    @empty<tr>
                        <td colspan="6" class="empty">No promo codes created yet.</td>
                    </tr>@endforelse
                </tbody>
            </table>
        </div>
