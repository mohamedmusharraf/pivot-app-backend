...
$chartRaw = \App\Models\GoalLogs::orderBy('created_at', 'asc')->limit(7)->get();
$chartLabels = $chartRaw->map(fn($gl) => 'Goal #' . $gl->id)->toArray();
$chartData = [
'target' => $chartRaw->pluck('target_minutes')->map(fn($minutes) => (int) $minutes)->values()->toArray(),
'achieved' => $chartRaw->pluck('achieved_minutes')->map(fn($minutes) => (int) $minutes)->values()->toArray(),
];
            ...
            chartConfig = {
                ...
                data: {
                    ...
                    datasets: [{
                            ...
                            data: dataValues.target.map(Number),
                            backgroundColor: '#3B5838',
                            borderRadius: 6
                        },
                        {
                            label: 'Achieved Minutes',
                            data: dataValues.achieved.map(Number),
